...
- The current user-selected root model and effort are authoritative. Installation defaults to **GPT-6 Sol / xhigh**, but the user may select **GPT-6 Astra** (or another available root) directly; routing never changes it.
...
- For ordinary work objectives, delegated work uses only `luna_worker`, pinned to **GPT-6 Luna / max**.
