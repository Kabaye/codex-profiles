...
| work automatically upgrades root or expensive children | Selector choice authoritative; only Luna roles installed; old Sol gate removed | External config/role changes can override |
...
| Parallelism merely multiplies work | Open-child cap 1/2; no slot-filling; one Astra child policy | One-Astra limit is not a separate backend counter |
...
2. In x20-work, manually select Astra root. Perform a small delegated read; verify Luna Max and no authorization phrase/marker. In a fresh Sol-root thread, verify the same default. No Sol/Astra child role should be installed by this profile.
...
5. Verify the normal root selector is no longer hidden by the old routing-owned catalog. A corporate restriction is not a defect to bypass. Confirm no stale Sol role in lite/x5/work, and no stale Luna role in x20.
