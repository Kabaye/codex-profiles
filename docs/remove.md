...
For a **profile switch**, follow the destination [installation procedure](install.md): it synchronizes roles, defaults, catalog policy, memory keys and the single routing block. Do not stack profiles.
...
Restart Codex and open a new thread. After complete removal, Codex's own model catalog/defaults apply unless another user/project/managed configuration intentionally restricts them.
