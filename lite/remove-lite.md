...
This removes the active repository-owned profile state, including the full managed `lite` AGENTS block, Luna worker/compatibility roles, profile-owned config keys and the repository-specific `models-lite.json` catalog. Unrelated configuration and unrelated role files are preserved.
