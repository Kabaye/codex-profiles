...
Follow the shared [installation and migration procedure](../docs/install.md) with profile `x20-work`. It covers backups, model metadata validation, role cleanup, config merging and live smoke tests. Do not install by copying only AGENTS.md.
...
```powershell
python scripts/manage_roles.py install x20-work
```
